...
class NotifyMeApp:
    ...
    def _on_blink_reminder(self) -> None:
        ...
        if not self.config.blink_hidden:
            ...
            self.notifications.show_blink_notification(
    ...
    def _on_walking_reminder(self) -> None:
        ...
        if not self.config.walking_hidden:
            ...
            self.notifications.show_walking_notification(
    ...
    def _on_water_reminder(self) -> None:
        ...
        if not self.config.water_hidden:
            ...
            self.notifications.show_water_notification(
    ...
    def _on_pranayama_reminder(self) -> None:
        ...
        if not self.config.pranayama_hidden:
            ...
            self.notifications.show_pranayama_notification(
    ...
    def toggle_sound(self) -> None:
        ...
        logging.info("Global sound %s", "enabled" if self.config.sound_enabled else "disabled")
        self.update_menu()
    ...
    def toggle_pranayama_sound(self) -> None:
        ...
        logging.info("Pranayama sound %s",
    ...
    def toggle_blink_hidden(self) -> None:
        ...
        logging.info("Blink reminder %s",
                     "hidden" if self.config.blink_hidden else "visible")
        self.update_menu()
    ...
    def toggle_walking_hidden(self) -> None:
        ...
        logging.info("Walking reminder %s",
                     "hidden" if self.config.walking_hidden else "visible")
        self.update_menu()

    def toggle_water_hidden(self) -> None:
        """Toggle water reminder visibility."""
        self.config.water_hidden = not self.config.water_hidden
        logging.info("Water reminder %s", "hidden" if self.config.water_hidden else "visible")
        self.update_menu()
    ...
    def toggle_pranayama_hidden(self) -> None:
        ...
        logging.info("Pranayama reminder %s",
    ...
    def update_icon_title(self) -> None:
        ...
        blink_status = "⏸" if self.timers.is_timer_paused(
            REMINDER_BLINK) else f"{self.config.interval_minutes}min"
        walk_status = "⏸" if self.timers.is_timer_paused(
            REMINDER_WALKING) else f"{self.config.walking_interval_minutes}min"
        water_status = "⏸" if self.timers.is_timer_paused(
            REMINDER_WATER) else f"{self.config.water_interval_minutes}min"
        pranayama_status = "⏸" if self.timers.is_timer_paused(
            REMINDER_PRANAYAMA) else f"{self.config.pranayama_interval_minutes}min"
    ...
    def run(self) -> None:
        ...
        try:
            while True:
                if not any(timer.is_running for timer in self.timers.timers.values()) and (
                    not self.icon or not self.icon.visible):
                    break
                time.sleep(0.5)
